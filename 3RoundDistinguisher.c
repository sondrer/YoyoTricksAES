...
/*
Proof of concept: Distinguisher for 3 rounds of AES using 3 texts
...
/*
  This routine repeats the 3round distinguisher 10 times for random pairs. 

  -picks a random key. 
  - genrates 10 pairs that are different in exactly one column
    for each test, it encrypts the texts 3 rounds, then pick a new ciphertext
   decrypt this and shows that the difference with the original texts is zero 
...
int main() {
  ...
  word8 c1[4][MAXBC];
  word8 c2[4][MAXBC];
  word8 p1_tmp[4][MAXBC];
  word8 p2_tmp[4][MAXBC];
